<template>
  <div class="myself-container">
    <div class="main">
      <div class="my">
        ...
        <div class="info">
          ...
          <div class="info-item">
            <h3>认证状态:</h3>
            <span>{{ myInfoList.identification === 0 ? '未认证' : myInfoList.identification === 1 ? '认证通过' : myInfoList.identification === 2 ? '认证失败' : '未知状态'  }}</span>
          </div>
          <div class="info-item">
            <h3>身份认证:</h3>
          </div>
    ...
    >
      ...
      >
        <img
          v-if="StuEditForm.imageData"
          :src="StuEditForm.imageData"
          class="edit-avatar"
        />
    ...
    >
      ...
      >
        <img
            v-if="StuEditForm.identityImage"
            :src="StuEditForm.identityImage"
            class="edit-avatar"
        />
        <i v-else class="el-icon-plus avatar-uploader-icon"></i>
      </el-upload>
...
export default {
  ...
  methods: {
    ...
    async excuteEditImgForm() {
      const { data: res } = await editStuImg(this.StuEditForm);
      if (res.code !== 200) return this.$message.error(res.message);
      //刷新
      sessionStorage.setItem("imageData", this.StuEditForm.imageData);
      this.getInfo();
    ...
    changeImageFn() {
      this.StuEditForm.imageData = this.myInfoList.imageData;
    ...
    async getInfo() {
      ...
      this.StuEditForm.uid = res.data.uid;
      this.StuEditForm.imageData = res.data.imageData;
    },

    changeIdenImageFn() {
      this.StuEditForm.identityImage = this.myInfoList.identityImage;
    ...
    handleAvatarChangeIdenIcon(file, fileList) {
      ...
      reader.readAsDataURL(file.raw);
      reader.onload = function (e) {
        This.StuEditForm.identityImage = this.result;
      };
    },
    ...
    async excuteEditIdenImgForm() {
      ...
      if (res.code !== 200) return this.$message.error(res.message);
      //刷新
      sessionStorage.setItem("identityImage", this.StuEditForm.identityImage);
      this.getInfo();
