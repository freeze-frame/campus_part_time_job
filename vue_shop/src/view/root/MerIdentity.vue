<template>
  <div class="myMerManage_container">
    <el-col :span="19">
      <el-form :inline="true">
...
<script>
import {businessShowAPI, businessEditAPI, businessDelAPI, businessSearchAPI, stuEditAPI} from '@/api/AxiosAPI';
...
export default {
  ...
  methods: {
    ...
    async getBusinessList() {
      ...
      this.businessList = res.data.records.filter(mer => mer.identification === 0);
    ...
    async updateIdentification(updatedRow) {
      ...
      const { data: res } = await businessEditAPI(updatedRow);
      if (res.code !== 200) {
        return this.$message.error(res.message);
      }
      this.$message.success(res.message);
